...
static inline void raw_nat_from_node_info(struct f4fs_nat_entry *raw_ne,
						struct node_info *ni)
{
...
static inline void next_free_nid(struct f4fs_sb_info *sbi, nid_t *nid)
{
	struct f4fs_nm_info *nm_i = NM_I(sbi);
 ...
 * inline functions
 */
static inline void get_nat_bitmap(struct f4fs_sb_info *sbi, void *addr)
{
	struct f4fs_nm_info *nm_i = NM_I(sbi);
...
static inline pgoff_t current_nat_addr(struct f4fs_sb_info *sbi, nid_t start)
{
	struct f4fs_nm_info *nm_i = NM_I(sbi);
	pgoff_t block_off;
	pgoff_t block_addr;
	...
	if (f4fs_test_bit(block_off, nm_i->nat_bitmap))
		block_addr += sbi->blocks_per_seg;

	return block_addr;
}

static inline pgoff_t next_nat_addr(struct f4fs_sb_info *sbi,
						pgoff_t block_addr)
{
	struct f4fs_nm_info *nm_i = NM_I(sbi);
...
static inline void set_to_next_nat(struct f4fs_nm_info *nm_i, nid_t start_nid)
{
	unsigned int block_off = NAT_BLOCK_OFFSET(start_nid);

	f4fs_change_bit(block_off, nm_i->nat_bitmap);
#ifdef CONFIG_F4FS_CHECK_FS
	f4fs_change_bit(block_off, nm_i->nat_bitmap_mir);
#endif
}

static inline nid_t ino_of_node(struct page *node_page)
{
	struct f4fs_node *rn = F4FS_NODE(node_page);
...
{
	struct f4fs_node *rn = F4FS_NODE(page);
...
{
	struct f4fs_node *src_rn = F4FS_NODE(src);
	struct f4fs_node *dst_rn = F4FS_NODE(dst);
	memcpy(&dst_rn->footer, &src_rn->footer, sizeof(struct node_footer));
}

static inline void fill_node_footer_blkaddr(struct page *page, block_t blkaddr)
{
	struct f4fs_checkpoint *ckpt = F4FS_CKPT(F4FS_P_SB(page));
	struct f4fs_node *rn = F4FS_NODE(page);
...
static inline bool is_recoverable_dnode(struct page *page)
{
	...
	/* Don't care crc part, if fsck.f4fs sets it. */
...
/*
 * f4fs assigns the following node offsets described as (num).
 * N = NIDS_PER_BLOCK
 *
 *  Inode block (0)
 *    |- direct node (1)
...
{
	struct f4fs_node *rn = F4FS_NODE(p);

	f4fs_wait_on_page_writeback(p, NODE, true, true);
 ...
 *  - Mark cold files in f4fs_inode_info
...
#ifdef CONFIG_F4FS_CHECK_FS
	f4fs_inode_chksum_set(F4FS_P_SB(page), page);
#endif
}
