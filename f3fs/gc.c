...
do_gc:
		...
		sync_mode = F3FS_OPTION(sbi).bggc_mode == BGGC_MODE_SYNC;

		/* foreground GC was been triggered via f3fs_balance_fs() */
		if (foreground)
			sync_mode = false;
		...
		trace_f3fs_background_gc(sbi->sb, wait_ms,
				prefree_segments(sbi), free_segments(sbi));

		/* balancing f3fs's metadata periodically */
...
static unsigned int get_cb_cost(struct f3fs_sb_info *sbi, unsigned int segno)
{
	...
	unsigned char u;
	unsigned int i;
	unsigned int usable_segs_per_sec = f3fs_usable_segs_in_sec(sbi, segno);
...
static inline unsigned int get_gc_cost(struct f3fs_sb_info *sbi,
...
static struct victim_entry *attach_victim_entry(struct f3fs_sb_info *sbi,
				...
				bool left_most)
{
	...
	ve =  f3fs_kmem_cache_alloc(victim_entry_slab,
				GFP_NOFS, true, NULL);
...
static void insert_victim_entry(struct f3fs_sb_info *sbi,
				unsigned long long mtime, unsigned int segno)
{
	struct atgc_management *am = &sbi->am;
	struct rb_node **p;
	...
	p = f3fs_lookup_rb_tree_ext(sbi, &am->root, &parent, mtime, &left_most);
	attach_victim_entry(sbi, mtime, segno, parent, p, left_most);
}

static void add_victim_entry(struct f3fs_sb_info *sbi,
...
static struct rb_node *lookup_central_victim(struct f3fs_sb_info *sbi,
						struct victim_sel_policy *p)
{
	struct atgc_management *am = &sbi->am;
	struct rb_node *parent = NULL;
	...
	f3fs_lookup_rb_tree_ext(sbi, &am->root, &parent, p->age, &left_most);

	return parent;
}

static void atgc_lookup_victim(struct f3fs_sb_info *sbi,
						struct victim_sel_policy *p)
{
	struct sit_info *sit_i = SIT_I(sbi);
	struct atgc_management *am = &sbi->am;
...
next:
	...
	f3fs_bug_on(sbi, !vblocks || vblocks == sec_blocks);

	/* u = 10000 * x% * 40 */
	u = div64_u64(accu * (sec_blocks - vblocks), sec_blocks) *
							(100 - age_weight);

	f3fs_bug_on(sbi, age + u >= UINT_MAX);
 ...
 */
static void atssr_lookup_victim(struct f3fs_sb_info *sbi,
						struct victim_sel_policy *p)
{
	struct sit_info *sit_i = SIT_I(sbi);
	struct atgc_management *am = &sbi->am;
...
static void lookup_victim_by_age(struct f3fs_sb_info *sbi,
						struct victim_sel_policy *p)
{
...
static void release_victim_entry(struct f3fs_sb_info *sbi)
{
	struct atgc_management *am = &sbi->am;
	...
	f3fs_bug_on(sbi, am->victim_count);
	f3fs_bug_on(sbi, !list_empty(&am->victim_list));
}

static bool f3fs_pin_section(struct f3fs_sb_info *sbi, unsigned int segno)
{
	struct dirty_seglist_info *dirty_i = DIRTY_I(sbi);
	unsigned int secno = GET_SEC_FROM_SEG(sbi, segno);
...
static bool f3fs_pinned_section_exists(struct dirty_seglist_info *dirty_i)
{
	return dirty_i->pinned_secmap_cnt;
}

static bool f3fs_section_is_pinned(struct dirty_seglist_info *dirty_i,
						unsigned int secno)
{
	return dirty_i->enable_pin_section &&
		f3fs_pinned_section_exists(dirty_i) &&
		test_bit(secno, dirty_i->pinned_secmap);
}

static void f3fs_unpin_all_sections(struct f3fs_sb_info *sbi, bool enable)
{
	unsigned int bitmap_size = f3fs_bitmap_size(MAIN_SECS(sbi));

	if (f3fs_pinned_section_exists(DIRTY_I(sbi))) {
...
static int f3fs_gc_pinned_control(struct inode *inode, int gc_type,
							unsigned int segno)
{
	...
	if (gc_type != FG_GC)
		return -EBUSY;
	if (!f3fs_pin_section(F3FS_I_SB(inode), segno))
		f3fs_pin_file_control(inode, true);
	return -EAGAIN;
}
 ...
 */
static int get_victim_by_default(struct f3fs_sb_info *sbi,
    ...
    ktcond_print2(ttt, 15, 4);
		...
		if (unlikely(is_sbi_flag_set(sbi, SBI_CP_DISABLED))) {
			if (p.alloc_mode == LFS) {
				 ...
				 */
				if (!f3fs_segment_has_free_slot(sbi, segno))
					goto next;
			}
		}
...
#endif
#if PROF13_5
  ttt[1] = ttt[0] + p.min_cost;
  ttt[2] = ttt[1] + p.max_search;
  ktcond_print2(ttt, 15, 3);
#endif
...
static void add_gc_inode(struct gc_inode_list *gc_list, struct inode *inode)
{
	...
	new_ie = f3fs_kmem_cache_alloc(f3fs_inode_entry_slab,
					GFP_NOFS, true, NULL);
	new_ie->inode = inode;

	f3fs_radix_tree_insert(&gc_list->iroot, inode->i_ino, new_ie);
...
static void put_gc_inode(struct gc_inode_list *gc_list)
{
	...
	list_for_each_entry_safe(ie, next_ie, &gc_list->ilist, list) {
		...
		iput(ie->inode);
		list_del(&ie->list);
		kmem_cache_free(f3fs_inode_entry_slab, ie);
	}
}
...
next_step:
	...
	for (off = 0; off < usable_blks_in_seg; off++, entry++) {
		...
		/* block may become invalid during f3fs_get_node_page */
		if (check_valid_map(sbi, segno, off) == 0) {
			f3fs_put_page(node_page, 1);
			continue;
		}

		if (f3fs_get_node_info(sbi, nid, &ni, false)) {
			f3fs_put_page(node_page, 1);
			continue;
		}
		...
		err = f3fs_move_node_page(node_page, gc_type);
...
static bool is_alive(struct f3fs_sb_info *sbi, struct f3fs_summary *sum,
		struct node_info *dni, block_t blkaddr, unsigned int *nofs)
{
	...
	if (f3fs_get_node_info(sbi, nid, dni, false)) {
		f3fs_put_page(node_page, 1);
		return false;
	}

	if (sum->version != dni->version) {
		f3fs_warn(sbi, "%s: valid data with mismatched node version.",
			  __func__);
		set_sbi_flag(sbi, SBI_NEED_FSCK);
	}

	if (f3fs_check_nid_range(sbi, dni->ino)) {
		f3fs_put_page(node_page, 1);
		return false;
	}
	...
	if (source_blkaddr != blkaddr) {
		...
		if (unlikely(check_valid_map(sbi, segno, offset))) {
			if (!test_and_set_bit(segno, SIT_I(sbi)->invalid_segmap)) {
				f3fs_err(sbi, "mismatched blkaddr %u (source_blkaddr %u) in seg %u",
				...
				int gc_type, unsigned int segno, int off)
{
	...
	block_t newaddr;
	int err = 0;
	bool lfs_mode = f3fs_lfs_mode(fio.sbi);
	int type = fio.sbi->am.atgc_enabled && (gc_type == BG_GC) &&
				(fio.sbi->gc_mode != GC_URGENT_HIGH) ?
				CURSEG_ALL_DATA_ATGC : CURSEG_COLD_DATA;

	/* do not read out */
	page = f3fs_grab_cache_page(inode->i_mapping, bidx, false);
	if (!page)
		return -ENOMEM;
	...
	set_new_dnode(&dn, inode, NULL, NULL, 0);
	err = f3fs_get_dnode_of_data(&dn, bidx, LOOKUP_NODE);
	if (err)
		goto out;
	...
	if (lfs_mode)
		f3fs_down_write(&fio.sbi->io_order_lock);

	mpage = f3fs_grab_cache_page(META_MAPPING(fio.sbi),
	...
	if (!PageUptodate(mpage)) {
		...
		f3fs_update_iostat(fio.sbi, FS_DATA_READ_IO, F3FS_BLKSIZE);
		f3fs_update_iostat(fio.sbi, FS_GDATA_READ_IO, F3FS_BLKSIZE);
	...
	/* write target block */
	f3fs_wait_on_page_writeback(fio.encrypted_page, DATA, true, true);
	...
	if (clear_page_dirty_for_io(fio.encrypted_page))
		dec_page_count(fio.sbi, F3FS_DIRTY_META);
	...
	f3fs_update_iostat(fio.sbi, FS_GC_DATA_IO, F3FS_BLKSIZE);

	f3fs_update_data_blkaddr(&dn, newaddr);
...
recover_block:
	if (err)
		f3fs_do_replace_block(fio.sbi, &sum, newaddr, fio.old_blkaddr,
							true, true, true);
up_out:
	if (lfs_mode)
		f3fs_up_write(&fio.sbi->io_order_lock);
put_out:
	f3fs_put_dnode(&dn);
out:
	f3fs_put_page(page, 1);
...
enum MOVE_DATA_STATE {
  MOVE_DATA_STATE_START,
  MOVE_DATA_STATE_GET_LOCK_PAGE = MOVE_DATA_STATE_START,
  MOVE_DATA_STATE_GET_LOCK_PAGE_DONE,
  MOVE_DATA_STATE_OUT,
  MOVE_DATA_STATE_RETRY,
  MOVE_DATA_STATE_DONE,
};

struct move_data_arg {
  struct page *page;
  int err;
  struct get_lock_page_arg get_lock_page_arg;
  enum MOVE_DATA_STATE state;
  struct f3fs_io_info fio;
  bool is_dirty;
};
...
{
  switch (arg->state) {
    case MOVE_DATA_STATE_GET_LOCK_PAGE:
      {
        arg->page = f3fs_get_lock_data_page1(inode, bidx, true, &arg->get_lock_page_arg);
...
struct gc_data_arg {
  ...
  struct page* page;
  block_t start_bidx;
  struct move_data_arg move_data_arg;
#if PROF13_2
  ktime_t ttt[10];
#endif
};
  ...
  for (off = 0; off < usable_blks_in_seg; off++, entry++) {
    ...
    switch (args[off].state) {
      ...
      case GC_STATE_MOVE_DONE:
        {
#if PROF13_2
          args[off].ttt[4] = ktime_get_raw();
#endif
          if (read_count > 0) {
            break;
          }
#if PROF13_2
          args[off].ttt[5] = ktime_get_raw();
#endif
...
#if PROF13_2
          args[off].ttt[6] = ktime_get_raw();
          ktcond_print2(args[off].ttt, 2, 7);
#endif
				...
				bool force_migrate, char dst_hint)
{
  ...
  ttt[0] = ktime_get_raw();
	...
	if (f3fs_sb_has_blkzoned(sbi))
		end_segno -= sbi->segs_per_sec -
					f3fs_usable_segs_in_sec(sbi, segno);
  ...
  ttt[0] = ktime_get_raw();
      ...
      ttt[3] = ttt[2];
		...
		if (!PageUptodate(sum_page) || unlikely(f3fs_cp_error(sbi))) {
#if PROF12_2
      ttt[1] = ktime_get_raw();
      ttt[2] = ttt[1];
      ttt[3] = ttt[2];
		...
		if (type != GET_SUM_TYPE((&sum->footer))) {
			f3fs_err(sbi, "Inconsistent segment (%u) type [%d, %d] in SSA and SIT",
      ...
      ttt[3] = ttt[2];
		 ...
		 * - lock_page(sum_page)         - f3fs_replace_block
...
#if PROF12_2
  ttt[3] = ktime_get_raw();
  ktcond_print2(ttt, type==SUM_TYPE_NODE? 12:13, 4);
#endif
	}
#if PROF12_1
  ttt[2] = ktime_get_raw();
...
//  printk("free sections %d ", free_sections(sbi));
	if (gc_type == BG_GC && has_not_enough_free_secs(sbi, 0, 0)) {
		/*
  ...
  ttt[2] = ktime_get_raw();
	...
	/* f3fs_balance_fs doesn't need to do BG_GC in critical path. */
...
#if PROF200
  prev_valid_count = get_valid_blocks(sbi, segno, false);
#endif
	seg_freed = do_garbage_collect(sbi, segno, &gc_list, gc_type,
				gc_control->should_migrate_blocks, worker_idx);
#if PROF200
  cur_valid_count = get_valid_blocks(sbi, segno, false);
#endif
...
//    printk("warning!! skipped buffer explosed.  not freed and clear %d, prev %d cur %d", segno, prev_valid_count, cur_valid_count);
    mutex_lock(&dirty_i->seglist_lock);
...
#if PROF11_1
  ttt[3] = ktime_get_raw();
  ttt[4] = ttt[3] + total_freed;
  ktcond_print2(ttt, 12, 5);
#endif
...
{
	victim_entry_slab = f3fs_kmem_cache_create("f3fs_victim_entry",
...
static void init_atgc_management(struct f3fs_sb_info *sbi)
{
	struct atgc_management *am = &sbi->am;
...
static int free_segment_range(struct f3fs_sb_info *sbi,
				unsigned int secs, bool gc_only)
{
  ...
  printk("free segment range");
	...
	for (type = CURSEG_HOT_DATA; type < NR_CURSEG_PERSIST_TYPE; type++)
		f3fs_allocate_segment_for_resize(sbi, type, start, end);
	...
	if (next_inuse <= end) {
		f3fs_err(sbi, "segno %u should be free but still inuse!",
			 next_inuse);
		f3fs_bug_on(sbi, 1);
	}
out:
...
static void update_sb_metadata(struct f3fs_sb_info *sbi, int secs)
{
	struct f3fs_super_block *raw_sb = F3FS_RAW_SUPER(sbi);
	int section_count;
	int segment_count;
...
static void update_fs_metadata(struct f3fs_sb_info *sbi, int secs)
{
	int segs = secs * sbi->segs_per_sec;
	long long blks = (long long)segs * sbi->blocks_per_seg;
	long long user_block_count =
				le64_to_cpu(F3FS_CKPT(sbi)->user_block_count);
	...
	F3FS_CKPT(sbi)->user_block_count = cpu_to_le64(user_block_count + blks);
...
int f3fs_resize_fs(struct f3fs_sb_info *sbi, __u64 block_count)
{
	...
	old_block_count = le64_to_cpu(F3FS_RAW_SUPER(sbi)->block_count);
	...
	if (is_sbi_flag_set(sbi, SBI_NEED_FSCK)) {
		f3fs_err(sbi, "Should run fsck to repair first.");
		return -EFSCORRUPTED;
	}

	if (test_opt(sbi, DISABLE_CHECKPOINT)) {
		f3fs_err(sbi, "Checkpoint should be enabled.");
		return -EINVAL;
	}
	...
	if (shrunk_blocks + valid_user_blocks(sbi) +
		...
		F3FS_OPTION(sbi).root_reserved_blocks > sbi->user_block_count)
		err = -ENOSPC;
...
out_unlock:
  ...
  f3fs_down_write(&sbi->gc_lock);
	...
	if (err) {
		update_fs_metadata(sbi, secs);
		update_sb_metadata(sbi, secs);
		f3fs_commit_super(sbi, false);
	}
recover_out:
	if (err) {
		set_sbi_flag(sbi, SBI_NEED_FSCK);
		f3fs_err(sbi, "resize_fs failed, should run fsck to repair!");
