...
#define F3FS_MIN_SEGMENTS	9 /* SB + 2 (CP + SIT + NAT) + SSA + MAIN */
#define F3FS_MIN_META_SEGMENTS	8 /* SB + 2 (CP + SIT + NAT) + SSA */
...
static inline void sanity_check_seg_type(struct f3fs_sb_info *sbi,
						unsigned short seg_type)
{
	f3fs_bug_on(sbi, seg_type >= NR_PERSISTENT_LOG);
}
...
#define MAIN_BLKADDR(sbi)						\
	(SM_I(sbi) ? SM_I(sbi)->main_blkaddr : 				\
		le32_to_cpu(F3FS_RAW_SUPER(sbi)->main_blkaddr))
#define SEG0_BLKADDR(sbi)						\
	(SM_I(sbi) ? SM_I(sbi)->seg0_blkaddr : 				\
		le32_to_cpu(F3FS_RAW_SUPER(sbi)->segment0_blkaddr))
...
#define TOTAL_SEGS(sbi)							\
	(SM_I(sbi) ? SM_I(sbi)->segment_count : 				\
		le32_to_cpu(F3FS_RAW_SUPER(sbi)->segment_count))
...
#define f3fs_bitmap_size(nr)			\
	(BITS_TO_LONGS(nr) * sizeof(unsigned long))

#define SECTOR_FROM_BLOCK(blk_addr)					\
	(((sector_t)blk_addr) << F3FS_LOG_SECTORS_PER_BLOCK)
#define SECTOR_TO_BLOCK(sectors)					\
	((sectors) >> F3FS_LOG_SECTORS_PER_BLOCK)
...
//  struct rw_semaphore mtime_lock;        // *_mtime
  ...
  struct rw_semaphore last_victim_lock;  // last_victim
	...
	unsigned int last_victim[MAX_GC_WORKER][MAX_GC_POLICY]; /* last victim segment # */
};
...
/* Notice: The order of dirty type is same with CURSEG_XXX in f3fs.h */
enum dirty_type {
...
struct dirty_seglist_info {
	...
	unsigned long *victim_secmap2;		/* all GC victims */
...
struct curseg_info {
	...
	struct f3fs_summary_block *sum_blk;	/* cached summary block */
	struct rw_semaphore journal_rwsem;	/* protect journal area */
	struct f3fs_journal *journal;		/* cached journal info */
 ...
 * inline functions
 */
static inline struct curseg_info *CURSEG_I(struct f3fs_sb_info *sbi, int type)
{
				...
				struct page *page, unsigned int start)
{
  // sentry_only (read)
	...
	raw_sit = (struct f3fs_sit_block *)page_address(page);
...
static inline void __set_free(struct f3fs_sb_info *sbi, unsigned int segno)
{
	struct free_segmap_info *free_i = FREE_I(sbi);
	unsigned int secno = GET_SEC_FROM_SEG(sbi, segno);
	unsigned int start_segno = GET_SEG_FROM_SEC(sbi, secno);
	unsigned int next;
	unsigned int usable_segs = f3fs_usable_segs_in_sec(sbi, segno);
...
static inline void __set_inuse(struct f3fs_sb_info *sbi,
		unsigned int segno)
{
	struct free_segmap_info *free_i = FREE_I(sbi);
	unsigned int secno = GET_SEC_FROM_SEG(sbi, segno);
...
static inline void __set_test_and_free(struct f3fs_sb_info *sbi,
...
static inline void __set_test_and_inuse(struct f3fs_sb_info *sbi,
		unsigned int segno)
{
	struct free_segmap_info *free_i = FREE_I(sbi);
	unsigned int secno = GET_SEC_FROM_SEG(sbi, segno);
...
static inline void get_sit_bitmap(struct f3fs_sb_info *sbi,
		void *dst_addr)
{
	struct sit_info *sit_i = SIT_I(sbi);
...
static inline unsigned int reserved_segments(struct f3fs_sb_info *sbi)
{
	return SM_I(sbi)->reserved_segments +
			SM_I(sbi)->additional_reserved_segments;
}

static inline unsigned int free_sections(struct f3fs_sb_info *sbi)
{
...
static inline int reserved_sections(struct f3fs_sb_info *sbi)
{
	return GET_SEC_FROM_SEG(sbi, reserved_segments(sbi));
}

static inline bool has_curseg_enough_space(struct f3fs_sb_info *sbi,
			unsigned int node_blocks, unsigned int dent_blocks)
{
	...
	for (i = CURSEG_HOT_NODE; i <= CURSEG_COLD_NODE; i++) {
		...
		left_blocks = f3fs_usable_blks_in_seg(sbi, segno) -
...
static inline bool has_not_enough_free_secs(struct f3fs_sb_info *sbi,
...
static inline bool excess_prefree_segs(struct f3fs_sb_info *sbi)
{
	return prefree_segments(sbi) > SM_I(sbi)->rec_prefree_segments;
}

static inline int utilization(struct f3fs_sb_info *sbi)
{
...
/*
 * Sometimes f3fs may be better to drop out-of-place update policy.
 * And, users can control the policy through sysfs entries.
 * There are five policies with triggering conditions as follows.
 * F3FS_IPU_FORCE - all the time,
 * F3FS_IPU_SSR - if SSR mode is activated,
 * F3FS_IPU_UTIL - if FS utilization is over threashold,
 * F3FS_IPU_SSR_UTIL - if SSR mode is activated and FS utilization is over
 *                     threashold,
 * F3FS_IPU_FSYNC - activated in fsync path only for high performance flash
 *                     storages. IPU will be triggered only if the # of dirty
 *                     pages over min_fsync_blocks. (=default option)
 * F3FS_IPU_ASYNC - do IPU given by asynchronous write requests.
 * F3FS_IPU_NOCACHE - disable IPU bio cache.
 * F3FS_IPU_HONOR_OPU_WRITE - use OPU write prior to IPU write if inode has
 *                            FI_OPU_WRITE flag.
 * F3FS_IPU_DISABLE - disable IPU. (=default option in LFS mode)
 */
...
enum {
	...
	F3FS_IPU_ASYNC,
	F3FS_IPU_NOCACHE,
	F3FS_IPU_HONOR_OPU_WRITE,
};

static inline unsigned int curseg_segno(struct f3fs_sb_info *sbi,
		int type)
{
...
static inline unsigned char curseg_alloc_type(struct f3fs_sb_info *sbi,
		int type)
{
	struct curseg_info *curseg = CURSEG_I(sbi, type);
...
{
	f3fs_bug_on(sbi, segno > TOTAL_SEGS(sbi) - 1);
}

static inline void verify_fio_blkaddr(struct f3fs_io_info *fio)
{
 ...
 */
static inline int check_block_count(struct f3fs_sb_info *sbi,
		int segno, struct f3fs_sit_entry *raw_sit)
{
	bool is_valid  = test_bit_le(0, raw_sit->valid_map) ? true : false;
	int valid_blocks = 0;
	int cur_pos = 0, next_pos;
	unsigned int usable_blks_per_seg = f3fs_usable_blks_in_seg(sbi, segno);
	...
	if (unlikely(GET_SIT_VBLOCKS(raw_sit) != valid_blocks)) {
		f3fs_err(sbi, "Mismatch valid blocks %d vs. %d",
	...
	if (usable_blks_per_seg < sbi->blocks_per_seg)
		f3fs_bug_on(sbi, find_next_bit_le(&raw_sit->valid_map,
				sbi->blocks_per_seg,
	...
	if (unlikely(GET_SIT_VBLOCKS(raw_sit) > usable_blks_per_seg
		...
		f3fs_err(sbi, "Wrong valid blocks %d or segno %u",
...
#ifdef CONFIG_F3FS_CHECK_FS
	if (f3fs_test_bit(offset, sit_i->sit_bitmap) !=
			f3fs_test_bit(offset, sit_i->sit_bitmap_mir))
		f3fs_bug_on(sbi, 1);
#endif
...
#ifdef CONFIG_F3FS_CHECK_FS
	f3fs_change_bit(block_off, sit_i->sit_bitmap_mir);
#endif
}

static inline unsigned long long get_mtime(struct f3fs_sb_info *sbi,
						bool base_time)
{
	struct sit_info *sit_i = SIT_I(sbi);
...
static inline void set_summary(struct f3fs_summary *sum, nid_t nid,
...
static inline block_t start_sum_block(struct f3fs_sb_info *sbi)
{
	return __start_cp_addr(sbi) +
		le32_to_cpu(F3FS_CKPT(sbi)->cp_pack_start_sum);
}

static inline block_t sum_blk_addr(struct f3fs_sb_info *sbi, int base, int type)
{
	return __start_cp_addr(sbi) +
		le32_to_cpu(F3FS_CKPT(sbi)->cp_pack_total_block_count)
				- (base + 1) + type;
}

static inline bool sec_usage_check(struct f3fs_sb_info *sbi, unsigned int secno)
{
 ...
 */
static inline int nr_pages_to_skip(struct f3fs_sb_info *sbi, int type)
{
 ...
 */
static inline long nr_pages_to_write(struct f3fs_sb_info *sbi, int type,
...
static inline void wake_up_discard_thread(struct f3fs_sb_info *sbi, bool force)
{
